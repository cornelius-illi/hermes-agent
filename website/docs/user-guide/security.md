...
approvals:
  ...
  forbid_self_approval: false     # the requester may not answer their own prompt (buttons, /approve, "yes")
...
| `forbid_self_approval` | `false` | Gateway approvals in shared channels: when true, the user whose message triggered the dangerous command cannot answer their own prompt — the Discord Allow/Deny buttons, a typed `/approve` / `/deny`, and bare-word replies ("yes", "no", ...) are all refused for the requester; someone else in the allowlist must decide (four-eyes). Applies to admins too: with `require_admin_for_exec_approval` on, a different admin must approve. The guard is per prompt — a `session`/`always` grant by the other user still applies to the whole session key. Native approval buttons on other platforms and relay prompt replies are not covered. |
...
These bare-word replies are gated exactly like `/approve` and `/deny`: when `allow_admin_from` / `group_allow_admin_from` makes those commands admin-only, a non-admin's "yes" is refused rather than resolving the approval.
